...
def engineer_features(df):
    ...
    df["BATHROOM_INDEX"] = (
        df["FULLBATH"] + 0.5 * df["HALFBATH"] + 0.5 * df["BSMTFULLBATH"]
    )
...
def build_preprocessor(df, preprocessor=None):
    ...
    transformed = pd.DataFrame(
        arr, columns=scale_present + binary_present, index=feature_df.index
    )
...
def clean_test(df, state):
    ...
    df, _ = clean_nominal_qualitative(
        df, neighborhood_mapping=state["neighborhood_mapping"], is_train=False
    )
